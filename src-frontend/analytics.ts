...
export const initAnalyticsView = async () => {
    ...
    container.innerHTML = `
        <div style="margin-bottom: 1rem;">
            ...
            <select id="analytics-exercise-select" style="width: 100%; padding: 0.5rem; background: #2a2a2a; color: white; border: 1px solid #444; margin-top: 0.5rem;">
                ...
                ${exercises.map((e: any) => `<option value="${e.id}">${e.name}</option>`).join('')}
            </select>
        </div>
...
const renderChart = async (e: any) => {
    ...
    try {
        ...
        const data = history.map((h: any) => {
            ...
            return h.sets.reduce((sum: number, set: any) => sum + set.volume, 0);
        });
