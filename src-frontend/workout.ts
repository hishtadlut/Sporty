...
let activeWorkoutSets: any[] = [];
let exercises: any[] = [];
let timerInterval: any = null;
let secondsElapsed = 0;

export const initWorkoutView = async () => {
    exercises = await fetchAPI('/exercises');
    renderWorkoutForm();
};
...
const handleExerciseSelect = async (e: any) => {
    const exerciseId = e.target.value;
    const ghostContainer = document.getElementById('ghost-mode-container');
    if (!exerciseId || !ghostContainer) return;

    try {
        const history = await fetchAPI(`/workouts/history/${exerciseId}`);
        if (history && history.length > 0) {
            const lastWorkout = history[0];
            const setsHtml = lastWorkout.sets.map((s: any) =>
                `Set ${s.set_number}: ${s.reps} reps @ ${s.weight} (Vol: ${s.volume})`
            ).join('<br/>');
...
const handleAddSet = () => {
    const exSelect = document.getElementById('exercise-select') as HTMLSelectElement;
    ...
    if (!exSelect || !repsInput || !weightInput) return;
    ...
    const exerciseOptionText = (exSelect && exSelect.options && exSelect.options[exSelect.selectedIndex]) ? (exSelect.options[exSelect.selectedIndex] as HTMLOptionElement).text : '';
